...
import { ANO, formatarMoeda } from '../utils/calculos'
import { exportarHorasExtrasPDF } from '../utils/exportarPDF'
...
export default function HorasExtras() {
  ...
  function handleCalcular(e) {
    ...
    if (!bruto || bruto <= 0) { setErro('Informe o salário bruto.'); return }

    const cargaHoraria = parseInt(form.cargaHorariaMensal)
    if (!cargaHoraria || cargaHoraria <= 0) { setErro('Informe a carga horária mensal.'); return }

    const qtdHoras = parseFloat(form.quantidadeHoras)
    if (!qtdHoras || qtdHoras <= 0) { setErro('Informe a quantidade de horas extras.'); return }

    const percentual = form.percentualExtra === 'custom'
      ? parseFloat(form.percentualCustom)
      : parseFloat(form.percentualExtra)
    if (!percentual || percentual <= 0) { setErro('Informe o percentual de hora extra.'); return }
      ...
      <form onSubmit={handleCalcular} className="bg-[#1a1a2e] border border-white/5 rounded-2xl p-6 sm:p-8 mb-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          ...
          <div>
            ...
            <input
              type="number" name="salarioBruto" value={form.salarioBruto} onChange={handleChange}
              placeholder="Ex: 3000.00" step="0.01" min="0"
          ...
          <div>
            ...
            <select name="cargaHorariaMensal" value={form.cargaHorariaMensal} onChange={handleChange}
          ...
          <div>
            ...
            <input
              type="number" name="quantidadeHoras" value={form.quantidadeHoras} onChange={handleChange}
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {[
                ...
                <label key={opt.value}
                  className={`flex flex-col items-center p-2.5 rounded-xl border cursor-pointer transition-all text-center ${form.percentualExtra === opt.value
                      ? 'border-emerald-500/40 bg-emerald-500/5'
                      : 'border-white/5 bg-white/[0.02] hover:border-white/10'
                    }`}
                >
                  <input type="radio" name="percentualExtra" value={opt.value}
                    checked={form.percentualExtra === opt.value} onChange={handleChange} className="sr-only"
                  />
            ...
            {form.percentualExtra === 'custom' && (
              <input type="number" name="percentualCustom" value={form.percentualCustom} onChange={handleChange}
                placeholder="Ex: 75" min="1"
          ...
          <div className="sm:col-span-2">
            <label className="flex items-center gap-3 bg-white/[0.02] border border-white/5 rounded-xl px-4 py-3 cursor-pointer hover:border-white/10 transition-all">
              <input type="checkbox" name="calcularDSR" checked={form.calcularDSR} onChange={handleChange} className="accent-emerald-500 w-4 h-4" />
              <div>
          ...
          {form.calcularDSR && (
            <>
              <div>
                ...
                <input type="number" name="diasUteisMes" value={form.diasUteisMes} onChange={handleChange}
                  min="1" max="31"
                  className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-emerald-500/50 focus:ring-1 focus:ring-emerald-500/25 transition-all"
                />
              </div>
              <div>
                ...
                <input type="number" name="diasDescansoMes" value={form.diasDescansoMes} onChange={handleChange}
                  min="0" max="15"
                  className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-emerald-500/50 focus:ring-1 focus:ring-emerald-500/25 transition-all"
                />
              </div>
        ...
        <div id="resultado" className="space-y-4">
          ...
          <div className="bg-gradient-to-r from-amber-500/10 to-orange-500/10 border border-amber-500/20 rounded-2xl p-6 text-center">
            ...
            <p className="text-gray-400 text-sm mb-4">
              Baixe um relatório profissional com todos os detalhes das suas horas extras.
            </p>
            <button
              type="button"
              onClick={() => exportarHorasExtrasPDF(form, resultado, isPremium, sessao?.email)}
